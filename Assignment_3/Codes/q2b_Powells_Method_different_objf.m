clear all
clc
obj_f= @(x) ( (x(1)+2.*x(2)-7).^2+(2.*x(1)+x(2)-5).^2 );
...
%% Plot
clf
x = linspace(-0.5 ,1.5,1500);
y = linspace(-0.5 ,5,1500);
[X,Y] = meshgrid(x,y);
Z=(X+2.*Y-7).^2+(2.*X+Y-5).^2;

 Levels=(fs(:,1))';
contour(X,Y,Z,'LevelList',[Levels 25:max(Levels)/75:max(Levels)])
  colorbar('vertical')
  grid on
xlabel("x1");
ylabel("x2");
title("Powell's Method f1");
